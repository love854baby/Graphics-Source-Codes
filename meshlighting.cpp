...
/*  
 *  - shows the effect of mesh on lighting
 *
 *  - a mesh of 1024 squares on the left
 *  - just one square on the right
 *  - hit w to see wireframe
 *  - GL_SMOOTH interpolates brightness/color values across a surface
 *    based on values of vertices
 *  - n to increase the number of squares of left square
 *  - N to decrease the number of squares of left square
...
static void drawtable(int n)
{
   ...
   glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, reddishcolor);
...
void keyboard(unsigned char key, int x, int y)
{
   ...
   switch (key) {
      ...
      case 'n':
         ...
         printf("Left square has %d squares\n", nleft*nleft);
         glutPostRedisplay();
         break;
